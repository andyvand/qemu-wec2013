...
public:
    CTXHeaders()
    {
		...
		m_VlanHeaderVA = nullptr;
		m_VirtioHeaderVA = nullptr;
		m_EthHeaderVA = nullptr;
		m_IPHeadersVA = nullptr;

		m_VlanHeaderPA = PHYSICAL_ADDRESS();
		m_VirtioHeaderPA = PHYSICAL_ADDRESS();
		m_EthHeaderPA = PHYSICAL_ADDRESS();
		m_IPHeadersPA = PHYSICAL_ADDRESS();

		m_MaxEthHeadersSize = 0;
	}
...
private:
    ...
    PVOID m_VlanHeaderVA;
    PVOID m_VirtioHeaderVA;
    PVOID m_EthHeaderVA;
    PVOID m_IPHeadersVA;

    PHYSICAL_ADDRESS m_VlanHeaderPA;
    PHYSICAL_ADDRESS m_VirtioHeaderPA;
    PHYSICAL_ADDRESS m_EthHeaderPA;
    PHYSICAL_ADDRESS m_IPHeadersPA;

    ULONG m_MaxEthHeadersSize;

    CTXHeaders(const CTXHeaders&);
    CTXHeaders& operator= (const CTXHeaders&);
};
...
private:
    ...
    ULONG m_UsedBuffersNum;
    CNB *m_NB;

    CTXDescriptor(const CTXDescriptor&);
    CTXDescriptor& operator= (const CTXDescriptor&);
...
private:
    ...
    CVirtQueue(const CVirtQueue&);
    CVirtQueue& operator= (const CVirtQueue&);
};
...
private:
    ...
    ULONG m_TotalDescriptors;
    ULONG m_FreeHWBuffers;
    //TODO: Temporary
    ULONG m_TotalHWBuffers;
    //TODO: Needs review
    bool m_DoKickOnNoBuffer;

    struct VirtIOBufferDescriptor *m_SGTable;
    ULONG m_SGTableCapacity;
    bool  m_Killed;
