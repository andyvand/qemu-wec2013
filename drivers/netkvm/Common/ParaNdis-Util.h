...
private:
	...
	CNdisSpinLock(const CNdisSpinLock&);
	CNdisSpinLock& operator= (const CNdisSpinLock&);
};
...
public:
    ...
    CLockedContext(T &LockObject, BOOLEAN Autolock = TRUE)
        : m_LockObject(LockObject), m_Autolock(Autolock) override
    {
        if (Autolock)
        {
            m_LockObject.Lock();
...
private:
	CLockedContext(const CLockedContext&);
	CLockedContext& operator= (const CLockedContext&);
};
...
private:
	...
	CPassiveSpinLockedContext(const CPassiveSpinLockedContext&) {};
	CPassiveSpinLockedContext& operator= (const CPassiveSpinLockedContext&) {};
};
...
private:
	...
	CDPCSpinLockedContext(const CDPCSpinLockedContext&) {};
	CDPCSpinLockedContext& operator= (const CDPCSpinLockedContext&) {};
};
...
LONG __inline NetKvmInterlockedAdd(
...
public:
	...
	CNdisRefCounter(const CNdisRefCounter&) {};
	CNdisRefCounter& operator= (const CNdisRefCounter&) {};
};
...
private:
	...
	CRefCountingObject(const CRefCountingObject&) {};
	CRefCountingObject& operator= (const CRefCountingObject&) {};
};
...
private:
    ...
    TEntryType *Pop_LockLess()
    {
        ...
        CounterDecrement();
        return (TEntryType *)TEntryType::GetByListEntry(RemoveHeadList(&m_List));
    }
...
public:
    ...
    CDpcIrqlRaiser(const CDpcIrqlRaiser&) ;
    CDpcIrqlRaiser& operator= (const CDpcIrqlRaiser&);
...
private:
    ...
    PVOID m_VA;
    NDIS_PHYSICAL_ADDRESS m_PA;
    ULONG m_Size;
    bool m_IsCached;

    CNdisSharedMemory(const CNdisSharedMemory&);
    CNdisSharedMemory& operator= (const CNdisSharedMemory&);
};
...
void ParaNdis_PrintTable(int DebugPrintLevel, TTable table, size_t Size, LPCSTR Format, AccessorsFuncs Accessors)
{
    ...
    if (StringCbCatA(FullFormat, sizeof(FullFormat), Format) != STATUS_SUCCESS)
    {
    ...
    while (i < Size)
    {
        ...
        {
            Res = StringCbPrintfExA(LinePos, ColumnWidth, &End, NULL, STRSAFE_FILL_ON_FAILURE | ' ',
